  ...
  const toggleStart = (e) => {
   ...
   useEffect(() => {
    if (openApp.includes(8)) {
      window.open('https://www.linkedin.com/in/kasturi-lal-singla-594053356/', '_blank');
      setOpenApp(openApp.filter(appId => appId !== 8));
    }
    if (openApp.includes(7)) {
      window.open('https://github.com/singlakasturi', '_blank');
      setOpenApp(openApp.filter(appId => appId !== 7));
    }
    if (openApp.includes(4)) {
      window.open('https://drive.google.com/file/d/1EreuVZJSo9VAEilbW4AgRNId7afIW4gz/view?usp=sharing');
      setOpenApp(openApp.filter(appId => appId !== 4));
    }
    if (openApp.includes(5)) {
      window.open('https://leetcode.com/u/SinglaKasturi/', '_blank');
